...
function boilerplate_load_assets() {
  ...
  // Enqueue lead magnet script only on homepage
  if (is_front_page()) {
    wp_enqueue_script('lead-magnet', get_theme_file_uri('/src/scripts/lead-magnet.js'), array(), '1.0', true);
  }
}
...
// Fix title tag - replace keyword stuffing with proper business title
function custom_title_tag($title) {
    if (is_front_page()) {
        return 'Local Vocal Marketing | Web Design & SEO for Maryland Small Businesses';
    }
    return $title;
}
add_filter('wp_title', 'custom_title_tag', 10, 1);

// Alternative title filter for newer WordPress versions
function custom_document_title($title) {
    if (is_front_page()) {
        $title['title'] = 'Local Vocal Marketing | Web Design & SEO for Maryland Small Businesses';
    }
    return $title;
}
add_filter('document_title_parts', 'custom_document_title', 10, 1);